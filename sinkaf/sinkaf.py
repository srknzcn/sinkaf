...
class Sinkaf:
    ...
    def __init__(self, model=LINEAR_MODEL_NAME):
        ...
        if self.model == Sinkaf.LINEAR_MODEL_NAME:
            self.clf = joblib.load(urlopen("https://github.com/srknzcn/sinkaf/blob/master/sinkaf/data/model_linearSVC.joblib?raw=true"))
        elif self.model in Sinkaf.BERT_MODEL_NAMES:
            if self.model == Sinkaf.BERT_HIGH_PRECISION_MODEL_NAME:
                self.clf = joblib.load(urlopen("https://github.com/srknzcn/sinkaf/blob/master/sinkaf/data/clf_nn_precision.joblib?raw=true"))
            elif self.model == Sinkaf.BERT_HIGH_RECALL_MODEL_NAME:
                self.clf = joblib.load(urlopen("https://github.com/srknzcn/sinkaf/blob/master/sinkaf/data/clf_nn_recall.joblib?raw=true"))
